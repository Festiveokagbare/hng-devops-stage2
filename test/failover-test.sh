...
# Loads variables from .env (if present) to know ports and active pool
if [ -f .env ]; then
...
# 2) Trigger chaos on active app (grader uses direct port)
...
# 3) Request loop for ~TEST_DURATION seconds
...
while [ $SECONDS -lt $end ]; do
  ...
  # little delay to not slam too fast
  sleep 0.1
done
...
# After chaos: at least 95% responses should be from the backup (green if blue was active)
if [ "$ACTIVE_POOL" = "blue" ]; then
  primary=blue
  backup=green
else
...
pct=$(awk -v b="$backup_count" -v t="$total" 'BEGIN{ if(t==0) print 0; else printf("%.2f", (b/t)*100) }')
echo "Backup responses percentage: ${pct}% (need >=95%)"
